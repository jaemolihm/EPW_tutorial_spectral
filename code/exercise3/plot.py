...
ws, es, sigma = parse_epw_specfun_sup(f"data2/specfun_sup.elself.{T:.3f}K")
ws, As, nocc = parse_epw_specfun(f"data2/specfun.elself.{T:.3f}K")
...
im = NonUniformImage(axes[0, 0], interpolation='nearest',
    ...
    cmap="Blues", norm=plt.matplotlib.colors.LogNorm(vmin=0.001, vmax=1))
...
plt.axhline(-1.0 - -4.1860, c="gray", ls="--", label='ahc_win_min, max')
plt.axhline(-6.5 - -4.1860, c="gray", ls="--")
...
# __TODO__ set the indices of the high-symmetry points to match the path
# defined in kpt.txt. For a Gamma-M-K-Gamma path with 100 points per segment
# one would use xs[[0, 100, 200, 300]] and labels below.
xs_highsym = xs[np.array([0, 100, 158, 273])]
for x in xs_highsym:
    for ax in axes[:, 0]:
        ax.axvline(x, c="k", lw=1)
plt.xticks(xs_highsym, ["$\Gamma$", "M", "K", "$\Gamma$"])
plt.xlim([xs[0], xs[-1]])


plt.tight_layout()
fig.savefig("graphene_spectral.pdf")
print("Saved figure to graphene_spectral.pdf")
...
fig_zoom.savefig("graphene_spectral_zoom.pdf")
print("Saved figure to graphene_spectral_zoom.pdf")
#plt.show()
